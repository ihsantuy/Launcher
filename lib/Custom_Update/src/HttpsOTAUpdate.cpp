...
void https_ota_task(void *param) {
    ...
    esp_err_t ret = esp_https_ota((const esp_https_ota_config_t *)param);
    if (ret == ESP_OK) {
        if (ota_status) {
...
HttpsOTAStatus_t HttpsOTAUpdateClass::status() {
    if (ota_status) {
        ...
        if (set_bit == OTA_IDLE_BIT) { return HTTPS_OTA_IDLE; }
        if (set_bit == OTA_UPDATING_BIT) { return HTTPS_OTA_UPDATING; }
        if (set_bit == OTA_SUCCESS_BIT) { return HTTPS_OTA_SUCCESS; }
        if (set_bit == OTA_FAIL_BIT) { return HTTPS_OTA_FAIL; }
    }
...
void HttpsOTAUpdateClass::begin(const char *url, const char *cert_pem, bool skip_cert_common_name_check) {
    ...
    if (!ota_status) {
        ...
        if (!ota_status) { log_e("OTA Event Group Create Failed"); }
